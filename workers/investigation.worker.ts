...
import { generateCommitMessage } from '@/lib/llm/commit';
  ...
  try {
    ...
    if (analysis.confidence > 80 && gitHubConfig.length > 0 && autoPrEnabled) {
      try {
        ...
        // Create PR
        const patchChanges = extractPatchChanges(analysis.patchDiff).map(
...
### Changes
${patchChanges || '1. Applied fix to ' + analysis.affectedFile}
