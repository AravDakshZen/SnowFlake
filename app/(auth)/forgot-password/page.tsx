...
export default function ForgotPasswordPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSent(true)
      toastSuccess('Reset link sent', 'Check your inbox for the password reset link.')
    } catch (cause) { const message = cause instanceof Error ? cause.message : 'Unable to send reset email'; setError(message); toastError('Could not send reset email', message) } finally { setLoading(false) }
  }
