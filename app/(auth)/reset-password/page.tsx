...
export default function ResetPasswordPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (password.length < 8 || password !== confirm) { setError(password !== confirm ? 'Passwords do not match' : 'Use at least 8 characters'); return }
    setLoading(true)
    try {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to reset password') } finally { setLoading(false) }
  }
  return <AuthShell mode="reset" eyebrow="Account recovery" title="Choose a new password" description="Use a strong password you do not reuse across production tools.">
    <form onSubmit={submit} className="mt-8 flex flex-col gap-4">{error && <p role="alert" className="rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">{error}</p>}{message && <p className="rounded-xl border border-primary/20 bg-primary/5 px-4 py-3 text-sm text-primary">{message}</p>}
    ...
    </form><div className="mt-7 flex items-center justify-center gap-2 text-sm text-muted-foreground"><ShieldCheck className="size-4" /> Reset links are single-use</div><p className="mt-4 text-center text-sm"><Link href="/signin" className="text-muted-foreground underline-offset-4 hover:text-foreground hover:underline">Back to sign in</Link></p>
  </AuthShell>
}
