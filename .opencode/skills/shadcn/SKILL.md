...
First, run `npx shadcn@latest info --json` and read the output — it contains the project config (aliases, base, style, icon library, Tailwind version, resolved paths, package manager) and the list of installed components. Keep it in context for the rest of the task. Refresh with `npx shadcn@latest info` if the project state changes. Use `npx shadcn@latest docs <component>` to get documentation and example URLs for any component.
